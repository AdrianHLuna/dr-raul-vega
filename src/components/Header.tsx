...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#0A192F] text-white shadow-xl border-b border-[#172A45]">
      ...
      <div className="bg-[#071324] text-slate-300 py-2 px-6 border-b border-slate-800 text-xs hidden sm:block">
      ...
      <div className="max-w-7xl mx-auto px-6 py-3.5 flex items-center justify-between">
        ...
        {/* Brand Container - LOGO-07 Clean Mark (No White Border) + Crisp HTML Typography */}
        <Link href="/" className="flex items-center gap-3.5 group py-0.5">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={headerLogoSrc}
            alt={`Logotipo ${doctor.title} ${doctor.name}`}
            className="h-11 sm:h-12 w-auto object-contain flex-shrink-0 group-hover:scale-105 transition-transform duration-300 drop-shadow-md"
          />

          <div className="flex flex-col">
            <h1 className="font-black text-base sm:text-lg text-white leading-tight tracking-wide group-hover:text-[#00A896] transition-colors">
              {doctor.title} {doctor.name}
            </h1>
            <p className="text-[10px] sm:text-[11px] text-[#00A896] font-extrabold tracking-wider uppercase">
        ...
        <div className="hidden sm:flex items-center gap-3">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="px-5 py-2.5 rounded-xl bg-[#00A896] text-[#0A192F] font-black text-xs uppercase tracking-wider flex items-center gap-2 hover:bg-[#02C39A] hover:scale-105 transition-all shadow-md"
          >
        ...
        <button
          ...
          className="lg:hidden p-2.5 rounded-xl bg-[#172A45] text-white hover:bg-[#00A896] hover:text-[#0A192F] transition-colors"
